...
class Quote
{
    ...
    public function calculate(string \$type, array \$selections): array
    {
        ...
        foreach (\$options as \$group => \$opts) {
            if (!isset(\$selections[\$group])) continue;
            \$sel = \$selections[\$group];
            // Múltiples (checkbox)
            if (is_array(\$sel)) {
                foreach (\$sel as \$val) {
                    foreach (\$opts as \$opt) {
        ...
        if (\$extraPages > 0) {
            \$items[] = ['desc' => "Páginas extra (\$extraPages)",
                        'price' => \$extraPages * self::EXTRA_PAGE_PRICE];
            \$total += \$extraPages * self::EXTRA_PAGE_PRICE;
        }

        // 4) Productos extra (solo ecommerce)
        \$extraProd = (int) (\$selections['extra_products'] ?? 0);
        if (\$extraProd > 0) {
            \$blocks = \$extraProd / 50;
            \$items[] = ['desc' => "Productos extra (\$extraProd)",
                        'price' => \$blocks * self::EXTRA_PRODUCTS_BLOCK];
